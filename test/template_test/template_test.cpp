...
TEST(Template, Expressions) {
  ...
  auto path = AliothHome() / "test" / "template_test" / "expr.tmpl";
...
TEST(Template, Branch) {
  ...
  auto path = AliothHome() / "test" / "template_test" / "branch.tmpl";
  auto result = Template::Render(path, model, meta);
...
TEST(Template, Iter) {
  ...
  auto path = AliothHome() / "test" / "template_test" / "iter.tmpl";
...
TEST(Template, Extends) {
  auto path = AliothHome() / "test" / "template_test" / "final.tmpl";
...
TEST(Template, Call) {
  auto path = AliothHome() / "test" / "template_test" / "caller.tmpl";
