...
bool Parser::ReduceOrFalse(Thread& thread) {
  ...
  if (formula == 0) {
    ...
    candidates_.push_back({AsNtrm(accept), thread.ignores});
    return true;
  }
...
ASTRoot Parser::AcceptOrAmbiguous() {
  if (candidates_.size() != 1) {
    ...
    for (auto const& [candidate, _] : candidates_) {
      ...
      fmt::println(stderr, "  {}", AttrsOf(candidate).dump(2));
    }
  ...
  root_->formula = 0;
  root_->attributes[NameOf(syntax)].push_back(candidate);

  /**
   * 将源码首末被忽略的单词填回句子
   */
  auto from = FirstOf(candidate)->offset;
  auto feeded = false;
...
void Parser::Crash() {
  ...
  for (auto const& thread : threads_) {
    ...
    fmt::println(stderr,
                 ...
                 LocationOf(input), NameOf(input), TextOf(input));
    if (!thread.seens.empty()) {
      auto const& last = thread.seens.back();
      fmt::println(stderr, "note: last symbol was {} ({})", NameOf(last),
                   TextOf(last));
    }
    std::vector<std::string> expected;
    auto const& state = root_->syntax->states.at(thread.stack.back());
    for (auto const& reduce : state.reduce)
      expected.push_back(NameOf(reduce.first, root_->syntax));
    for (auto const& shift : state.shift)
      expected.push_back(NameOf(shift.first, root_->syntax));
