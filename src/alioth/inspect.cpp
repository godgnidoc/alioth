...
FormulaID FinalFormulaOf(AST node) {
  ...
  while (syntax->formulas.at(ntrm->formula).Unfolded()) {
    ...
    ntrm = AsNtrm(ntrm->sentence.front());
  }
...
nlohmann::json AttrsOf(AST node, Skeleton const* skeleton) {
  if (auto term = AsTerm(node)) return TextOf(node);

  auto ntrm = AsNtrm(node);
  auto root = ntrm->root.lock();
  auto syntax = root->syntax;
  auto const& final_formula = syntax->formulas.at(FinalFormulaOf(ntrm));

  Skeleton::Attributes const* hints{};
  if (skeleton) {
    auto const& structure = skeleton->structures.at(ntrm->id);
    if (final_formula.form) {
      hints = &structure.forms.at(*final_formula.form).attributes;
    } else {
  ...
  for (auto const& [name, values] : ntrm->attributes) {
    ...
    if (is_single) {
      attrs[name] = AttrsOf(values.front(), skeleton);
      continue;
    }

    nlohmann::json attr;
    for (auto const& value : values) {
      attr.push_back(AttrsOf(value, skeleton));
    }
    attrs[name] = attr;
  }
