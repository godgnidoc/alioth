    ...
    for (auto const& item : itemset) {
      ...
      if (state.shift.count(item.ahead)) {
        fmt::println(stderr, "shift reduce conflict: {}",
                     NameOf(item.ahead, syntax_));
        fmt::println(stderr, "Reduce: {}", PrintFormula(item.formula, syntax_));
        fmt::println(stderr, "Shift:");
        for (auto const& it : itemset) {
          if (it.ahead != item.ahead) continue;

          fmt::println(stderr, "  {}",
                       PrintFormula(it.formula, syntax_, it.point));
        }
        fmt::println(stderr, "state: {}", PrintState(state_id, syntax_));
      ...
      if (state.reduce.count(item.ahead)) {
        ...
        fmt::println(stderr, "Reduce: {}",
                     PrintFormula(state.reduce.at(item.ahead), syntax_));
        fmt::println(stderr, "state: {}", PrintState(state_id, syntax_));
