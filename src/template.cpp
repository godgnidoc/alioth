...
Tmpl Template::Load(Doc source) {
  ...
  auto tnode = AttrOf(root, "template");
  auto tmpl = std::make_shared<Template>();
  for (auto fnode : AttrsOf(tnode, "fragments")) {
...
Template::Frag Template::LoadFrag(AST node) {
  Frag f;
  if (auto text = AttrOf(node, "text"); text) {
    auto frag = std::make_shared<TextFragment>();
    frag->text = TextOf(text);
    f = frag;
  } else if (auto call = AttrOf(node, "call"); call) {
    auto frag = std::make_shared<CallFragment>();
    frag->call = nlohmann::json::parse(TextOf(call)).get<String>();
    auto model = AttrOf(node, "model");
    if (model) frag->model = LoadExpr(model);
    f = frag;
  } else if (auto eval = AttrOf(node, "eval"); eval) {
    auto frag = std::make_shared<EvalFragment>();
    frag->eval = LoadExpr(eval);
    f = frag;
  } else if (auto iter = AttrOf(node, "iter"); iter) {
    auto frag = std::make_shared<IterFragment>();
    frag->iter = LoadExpr(iter);
    frag->value = *TextOf(node, "value");
    if (auto key = AttrOf(node, "key"); key) {
      frag->key = TextOf(key);
    }
    if (auto index = AttrOf(node, "index"); index) {
      frag->index = TextOf(index);
    }
    for (auto fnode : AttrsOf(node, "fragments")) {
      frag->fragments.push_back(LoadFrag(fnode));
    }
    if (auto anchor = AttrOf(node, "anchor"); anchor) {
      frag->anchor = TextOf(anchor);
    }
    f = frag;
  } else if (auto branch = AttrsOf(node, "branch"); !branch.empty()) {
    auto frag = std::make_shared<BranchFragment>();
    if (AttrOf(node, "trim_start")) frag->trim_start = true;
    if (AttrOf(node, "trim_end")) frag->trim_end = true;
    for (auto br : branch) {
      auto block = std::make_shared<BranchFragment::BranchBlock>();
      if (AttrOf(br, "trim_start")) block->trim_start = true;
      if (AttrOf(br, "trim_end")) block->trim_end = true;
      if (auto cond = AttrOf(br, "cond"); cond) block->cond = LoadExpr(cond);
      for (auto fnode : AttrsOf(br, "fragments")) {
    ...
    f = frag;
  } else if (auto block = AttrOf(node, "block"); block) {
    auto frag = std::make_shared<BlockFragment>();
    frag->block = TextOf(block);
    for (auto fnode : AttrsOf(node, "fragments")) {
      frag->fragments.push_back(LoadFrag(fnode));
    }
    f = frag;
  } else if (auto extends = AttrOf(node, "extends"); extends) {
    auto frag = std::make_shared<ExtendsFragment>();
    frag->extends = nlohmann::json::parse(TextOf(extends)).get<String>();
    for (auto overwrite : AttrsOf(node, "overwrites")) {
      frag->overwrites.push_back(LoadFrag(overwrite));
    }
    f = frag;
  } else if (auto overwrite = AttrOf(node, "overwrite"); overwrite) {
    auto frag = std::make_shared<OverwriteFragment>();
    frag->overwrite = TextOf(overwrite);
    for (auto fnode : AttrsOf(node, "fragments")) {
      frag->fragments.push_back(LoadFrag(fnode));
    }
    f = frag;
  ...
  if (AttrOf(node, "trim_start")) f->trim_start = true;
  if (AttrOf(node, "trim_end")) f->trim_end = true;
  return f;
}

Template::Expr Template::LoadExpr(AST node) {
  if (auto variable = AttrOf(node, "variable"); variable) {
    auto expr = std::make_shared<VariableExpr>();
    expr->variable = TextOf(variable);
    if (auto anchor = AttrOf(node, "anchor"); anchor) {
      expr->anchor = TextOf(anchor);
    }
    return expr;
  } else if (auto field = AttrOf(node, "field"); field) {
    auto expr = std::make_shared<FieldExpr>();
    expr->of = LoadExpr(AttrOf(node, "of"));
    expr->field = TextOf(field);
    return expr;
  } else if (auto index = AttrOf(node, "index"); index) {
    auto expr = std::make_shared<IndexExpr>();
    expr->of = LoadExpr(AttrOf(node, "of"));
    expr->index = LoadExpr(AttrOf(node, "index"));
    return expr;
  } else if (auto number = AttrOf(node, "number"); number) {
    auto text = TextOf(number);
  ...
  } else if (auto string = AttrOf(node, "string"); string) {
    auto expr = std::make_shared<StringExpr>();
    expr->string = nlohmann::json::parse(TextOf(string)).get<String>();
    return expr;
  } else if (auto invoke = AttrOf(node, "invoke"); invoke) {
    auto expr = std::make_shared<InvokeExpr>();
    expr->invoke = LoadExpr(AttrOf(node, "invoke"));
    for (auto param : AttrsOf(node, "params")) {
      expr->params.push_back(LoadExpr(param));
    }
    return expr;
  } else if (auto pipe = AttrOf(node, "pipe"); pipe) {
    auto expr = std::make_shared<PipeExpr>();
    expr->pipe = LoadExpr(AttrOf(node, "pipe"));
    expr->expr = LoadExpr(AttrOf(node, "expr"));
    return expr;
  }
...
Template::Filter Template::Compile(std::shared_ptr<BranchFragment> frag) {
  ...
  for (auto const& br : frag->branch) {
    if (br->cond)
      conds.push_back(Compile(*br->cond));
    else
      conds.push_back([](Context&, Array const&) { return Value(true); });
    std::vector<Filter> fragments;
